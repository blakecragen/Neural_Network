...
class NeuralNet:
    ...
    def _dropout(u, prob):
        ...
        return u * dropout_mask / (1.0 - prob), dropout_mask
    ...
    def cross_entropy_loss(self, y, y_hat):
        ...
        cross_entropy = -np.sum(y * np.log(y_hat), axis=1)
    ...
    def compute_gradients(self, y, yh, use_dropout):
        ...
        I tried but I kept messing up something. It was probably a super dumb typo but I ended up just retypign lolol
        """
        dLoss_u2 = yh - y
        ...
        if use_dropout:
            dLoss_u1 *= 1 / (1 - self.dropout_prob)
    ...
    def gradient_descent(self, x, y, iter=60000, use_momentum=False, local_test=False):
        ...
        **For LOCAL TEST append and print out loss at every iteration instead of every 1000th multiple.
        ...
        for i in range(iter):
            y_hat = self.forward(x, use_dropout=True if self.use_dropout else False)
            loss = self.cross_entropy_loss(y, y_hat)
            self.backward(y, y_hat, self.use_dropout, use_momentum)
